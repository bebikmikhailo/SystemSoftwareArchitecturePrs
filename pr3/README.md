...
При запуску програми від імені користувача root, обмеження ulimit -u 50 не спрацювало належним чином, оскільки ядро Linux надає суперкористувачу привілеї на перевищення лімітів процесів для забезпечення стабільності системи. Програма зупинилася на позначці 2226 через вичерпання віртуальної пам'яті (стеку потоків). Для коректної перевірки впливу ulimit -u необхідно запускати процес від імені не-привілейованого користувача.

Для цього потрібно прописати команди:
```
useradd -m tester
chown tester task1_var1
su tester -c "ulimit -u 50; ./task1_var1"
```
![](readme-images/8.png)

Як бачимо програма зупинилася на 48 потоках, бо ті 2 потоки пішли на оболонку bash та ще один допоміжний процес.
